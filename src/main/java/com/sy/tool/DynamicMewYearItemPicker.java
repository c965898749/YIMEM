...
import com.sy.model.game.MewYearItemShop;
...
public class DynamicMewYearItemPicker {
    // 动态物品池（支持新增物品）
    private List<MewYearItemShop> dynamicItemPool = new ArrayList<>();
    private Random random = new Random();

    // 新增物品到动态池
    public void addItem(MewYearItemShop item) {
        if (item != null) {
     ...
     * @return 随机物品列表
     */
    public List<MewYearItemShop> pickRandomItems(int count) {
        List<MewYearItemShop> result = new ArrayList<>();
        if (count <= 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // 3.2 筛选该品质下有库存的物品
            List<MewYearItemShop> candidates = new ArrayList<>();
            for (MewYearItemShop item : dynamicItemPool) {
            ...
            MewYearItemShop selected = candidates.get(
